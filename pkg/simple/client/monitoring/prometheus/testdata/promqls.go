...
var PromQLs = map[string]string{
	...
	"node_npu_utilization":                  `round(avg(npu_chip_info_utilization{node="i-2dazc1d6"}) by (node) / 100, 0.00001)`,
	...
	"pod_cpu_usage":                         `round(sum by (namespace, pod) (irate(container_cpu_usage_seconds_total{job=~"kubelet|kubeedge", pod!="", image!=""}[5m])), 0.001) *  on (namespace, pod) group_left (qos,owner_kind,owner_name,node) qos_owner_node:kube_pod_info:{owner_kind="ReplicaSet", owner_name=~"^elasticsearch-[^-]{1,10}$",pod=~"elasticsearch-0", namespace="default"}`,
	"pod_memory_usage":                      `sum by (namespace, pod) (container_memory_usage_bytes{job=~"kubelet|kubeedge", pod!="", image!=""}) * on (namespace, pod) group_left (qos,owner_kind,owner_name,node) qos_owner_node:kube_pod_info:{pod="elasticsearch-12345", namespace="default"}`,
	"pod_memory_usage_wo_cache":             `sum by (namespace, pod) (container_memory_working_set_bytes{job=~"kubelet|kubeedge", pod!="", image!=""}) * on (namespace, pod) group_left (qos,owner_kind,owner_name,node) qos_owner_node:kube_pod_info:{pod="elasticsearch-12345", node="i-2dazc1d6"}`,
	"pod_net_bytes_transmitted":             `sum by (namespace, pod) (irate(container_network_transmit_bytes_total{pod!="", interface!~"^(cali.+|tunl.+|dummy.+|kube.+|flannel.+|cni.+|docker.+|veth.+|lo.*)", job=~"kubelet|kubeedge"}[5m])) * on (namespace, pod) group_left (qos,owner_kind,owner_name,node) qos_owner_node:kube_pod_info:{namespace=~"logging|ks",pod=~"elasticsearch-0|redis"}`,
	"container_cpu_usage":                   `round(sum by (namespace, pod, container) (irate(container_cpu_usage_seconds_total{job=~"kubelet|kubeedge", container!="POD", container!="", image!="", pod="elasticsearch-12345", namespace="default", container="syscall"}[5m])), 0.001)`,
	"container_memory_usage":                `sum by (namespace, pod, container) (container_memory_usage_bytes{job=~"kubelet|kubeedge", container!="POD", container!="", image!="", pod="elasticsearch-12345", namespace="default", container=~"syscall"})`,
