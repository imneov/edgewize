...
func (c clustersGetter) Get(_, name string) (runtime.Object, error) {
	cluster, err := c.informers.Infra().V1alpha1().Clusters().Lister().Get(name)
	if err != nil {
		return nil, err
	}
...
func (c clustersGetter) List(_ string, query *query.Query) (*api.ListResult, error) {
	clusters, err := c.informers.Infra().V1alpha1().Clusters().Lister().List(query.Selector())
	if err != nil {
		return nil, err
	}
...
func (c clustersGetter) compare(left runtime.Object, right runtime.Object, field query.Field) bool {
	leftCluster, ok := left.(*infrav1alpha1.Cluster)
	if !ok {
		return false
	}

	rightCluster, ok := right.(*infrav1alpha1.Cluster)
	if !ok {
		return false
	}
...
func (c clustersGetter) filter(object runtime.Object, filter query.Filter) bool {
	cluster, ok := object.(*infrav1alpha1.Cluster)
	if !ok {
		return false
	}
