...
func New(informer informers.SharedInformerFactory) ResourceQuotaGetter {
	return &resourceQuotaGetter{informer: informer}
}

func (c *resourceQuotaGetter) getUsage(namespace, resource string) (int, error) {

	genericInformer, err := c.informer.ForResource(supportedResources[resource])
	if err != nil {
...
func (c *resourceQuotaGetter) getNamespaceResourceQuota(namespace string) (*v1.ResourceQuotaStatus, error) {
	resourceQuotaLister := c.informer.Core().V1().ResourceQuotas().Lister()
