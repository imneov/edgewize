...
import (
	...
	model "github.com/edgewize-io/edgewize/pkg/models/monitoring"
...
func AddToContainer(c *restful.Container, k8sClient kubernetes.Interface, monitoringClient monitoring.Interface, metricsClient monitoring.Interface, factory informers.InformerFactory, rtClient runtimeclient.Client) error {
	...
	h := NewHandler(k8sClient, monitoringClient, metricsClient, factory, nil, rtClient)
	...
	ws.Route(ws.GET("/ingresses").
		...
		Param(ws.QueryParameter("namespaced_resources_filter", "Specifies a namespaced resources filter in `<namespace>|<namespace>` format. For example, a namespaced resources filter like `ns1|ns2` will request the data of ns1 together with ns2.").DataType("string").Required(false)).
		Param(ws.PathParameter("ingress", "ingress name.").DataType("string").Required(false)).
