...
const (
	controllerName = "apptemplateversions-controller"
	Finalizer      = "apptemplateversions.finalizer.edge.edgewize.io"
)
...
func (r *Reconciler) SetupWithManager(mgr ctrl.Manager) error {
	klog.V(0).Info("[+]setup with manager")
	...
	return ctrl.NewControllerManagedBy(mgr).
		...
		For(&appsv1alpha1.AppTemplateVersion{}).
		Complete(r)
}

// +kubebuilder:rbac:groups=edge.edgewize.io,resources=apptemplateversion,verbs=get;list;watch;create;update;patch;delete
// +kubebuilder:rbac:groups=apps,resources=deployments,verbs=get;list;watch;delete
func (r *Reconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	logger := r.Logger.WithValues("AppTemplateVersion", req.NamespacedName)
	logger.V(7).Info("receive request", "req", req)
	rootCtx := context.Background()
	instance := &appsv1alpha1.AppTemplateVersion{}
	...
	if instance.ObjectMeta.DeletionTimestamp.IsZero() {
		if !sliceutil.HasString(instance.ObjectMeta.Finalizers, Finalizer) {
			...
			if err := r.UpdateInstance(rootCtx, req.NamespacedName, func(_instance *appsv1alpha1.AppTemplateVersion) error {
		...
		r.UpdateInstanceAndDeployments(ctx, instance)
	} else {
		...
		if sliceutil.HasString(instance.ObjectMeta.Finalizers, Finalizer) {
			...
			if err := r.UpdateInstance(rootCtx, req.NamespacedName, func(_instance *appsv1alpha1.AppTemplateVersion) error {
				...
				logger.V(4).Info("update apptemplateversion")
				return r.Update(rootCtx, _instance)
			}); err != nil {
				logger.Error(err, "update apptemplateversion failed")
				return ctrl.Result{}, err
			}
		}
...
func (r *Reconciler) UpdateInstance(ctx context.Context, nn types.NamespacedName, updateFunc func(deployer *appsv1alpha1.AppTemplateVersion) error) error {
...
func (r *Reconciler) undoReconcile(ctx context.Context, instance *appsv1alpha1.AppTemplateVersion) (ctrl.Result, error) {
	rootCtx := context.Background()
	var err error
	selector := labels.SelectorFromSet(map[string]string{appsv1alpha1.LabelAppTemplate: instance.Name})
